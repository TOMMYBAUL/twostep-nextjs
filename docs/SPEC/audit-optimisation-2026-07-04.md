...
| ~~P0-3~~ ✅ | **FAIT (2026-07-04, run autonome)** — `GET /api/products` (E3) paginé KEYSET via `fetchAllRows(() => …order("id"))`, filtres/population INCHANGÉS, tri par nom rétabli côté serveur (contrat UI). +captureError sur erreur de page/`maxPages` (diagnostic ops, comme les crons paginés) + `maxDuration=300`. Preuve `tests/products-route-pagination.test.ts` (1500 → 2 pages keyset, 1500 rendus, ordre nom, page-error→500). Revue SF-hunter SOUND (core) + 2 findings observabilité corrigés. 0 migration, réversible. | — | **[CONFIRMÉ]** | S |
...
| ~~P0-6~~ ✅ | **FAIT (2026-07-06, run autonome)** — RPC `(previous, written)` : les 4 webhooks + /api/stock n'émettent feed_event/notif QUE si `written` (le stock a vraiment changé) → plus de `restock` fantôme ni de push « de retour en stock » sur un absolu périmé rejeté par la garde 104. Wrapper dual-shape → code sûr AVANT la migration. **Migration 110** (RETURNS TABLE, SUBSUME la 109 P0-5, +RE-REVOKE anon/authenticated = ferme un trou ACL pré-existant, +lock_timeout) **PRÉPARÉE NON APPLIQUÉE, escaladée**. Revues silent-failure + database SOUND (1 CRITIQUE ACL + 1 HIGH lock + 1 MED + 1 LOW corrigés). Preuve : `migration-110-written-flag.test.ts` + `update-stock-wrapper.test.ts` + régressions written:false dans `webhook-routes-stock*`. 1232→1269. Réversible. | **[CONFIRMÉ]** | S |
| ~~P0-7~~ ✅ | **DÉJÀ FAIT (commit `6641034`, vérifié au code réel 2026-07-06)** : `resync-stock.ts` pagine KEYSET (`fetchAllRows`) les 2 lectures (produits + connexions) + batch upserts (chunk 500) + repli mono-ligne + statut honnête + captureError. | `resync-stock.ts` | — | **[CONFIRMÉ]** | S |
| ~~P0-8~~ ✅ | **FAIT (2026-07-05, run autonome)** — `collapseGtin14` dans `identifiers/validators.ts` : GTIN-14 **indicateur 0** (EAN-13 zéro-préfixé, checksum invariant) collapsé en EAN-13 interne AVANT classification dans `detectIdentifierType`/`canonicalizeEan` → même forme canonique que le POS/triage (fin du doublon e-invoice vs caisse). **Indicateur 1-9** (carton/palette = unité logistique distincte) **non** collapsé → suivi en SKU faible, jamais fusionné à tort. Finding SF-hunter HIGH in-scope **corrigé** : `cascade-engine.ts:198` stockait `kicks.gtin` (via `validEanOrNull`, non canonicalisé) direct dans `canonical_ean` → repassé par `canonicalizeEan` (même classe P0-8 dans le tier enrichissement sneakers). Preuve : `validators.test.ts` (+GTIN-14, cross-canal match), `ingest-triage.test.ts` (+régression e-invoice≡POS + carton→SKU), `cascade-engine.test.ts` (+kicks GTIN-14→collapsé + preflightEan). Revue SF-hunter **core SOUND** (invariance checksum prouvée math+empirique). 0 migration, réversible. **Résidus notés** (dormants, hors périmètre live) : validate.ts non fusionné (root cause dual-validator, §note), einvoice-cii/scan-session pas encore câblés à une route. | **[CONFIRMÉ]** | S |
...
| ~~P0-10~~ ✅ | **FAIT (2026-07-06, run autonome)** — `collectAllEanSources` ne court-circuite plus sur un cache mono-source FAIBLE (tier6=0.90 < auto_publish) : il conserve le tier caché (non-régression du score) et lance quand même les 4 sources parallèles → convergence tier6+tier2 détectée (0.90→0.985 → `validated`). **Write-back UPGRADE** : la convergence trouvée est ré-écrite dans `ean_lookups` (champs FUSIONNÉS sous la source forte, via `cacheResult`) → les appels suivants au même EAN court-circuitent fort (fin de la fuite de coût). `cacheResult` durci (capture `{error}` supabase-js, ferme un silent-swallow). Prémisse CONFIRMÉE empiriquement en prod (13/13 lignes cache = tier6). Preuve `tests/lib/enrichment/multi-source-convergence.test.ts` (+10, 1232→1242). Revue SF-hunter 2 passes (core SOUND + HIGH write-back + 2 findings delta corrigés). D7 intact → 0 faux positif. 0 migration, réversible. **Reste = C3** (dédup double-cascade = optim coût). | `enrichment/multi-source.ts` ; `ean/lookup.ts:821` | Convergence sur cache faible + write-back monotone | **[Fable]→vérifié** | M |
| ~~P0-11~~ ✅ | **FAIT (2026-07-05, run autonome)** — SKU match scopé `merchant_id` + garde longueur ≥4, unifié dans `src/lib/enrichment/sku-identity.ts` (`findSharedSkuEan`) → **2 instances dupliquées fermées** (`enrich-product.ts` + `resolve-ean.ts`, source unique = plus de divergence). Blip DB discriminé PGRST116 vs vrai outage (captureError, leçon E5). Preuve `tests/lib/enrichment/sku-identity.test.ts` (+10 : cross-marchand→null + verrou `.eq(merchant_id)`, même-marchand OK, <4→0 requête, PGRST116→0 Sentry, blip→Sentry). Revue SF-hunter **SOUND** (0 CRITICAL/HIGH, 3 callers `resolveAndEnrich` audités, MED blip fermé). 0 migration, réversible. | `enrichment/enrich-product.ts:46-54` | Scoper le match SKU par marchand + garde longueur SKU ≥ 4 | **[Fable]→vérifié** | M |
...
- **A1 [H×S] Webhook pousse TOUT l'inventaire à chaque vente** (M1#3 = M6#1, **confirmé par 2 agents**). `productIds` existe déjà (`inventory.ts:105-107`), les ids touchés sont en main. 5k SKU × 100 ventes/j = 500k appels/j → ~200. **Fix trivial, à faire avant le pilote.** ✅ **FAIT** (commit `6641034`).
- ~~**A2 [H×M] Aucun backoff/retry 429/5xx Google**~~ ✅ **FAIT (2026-07-05, run autonome)** — `googleMerchantFetch` réessaie 429/5xx (backoff exponentiel plein-jitter borné + honore `Retry-After`, `GOOGLE_MAX_RETRIES=3`) au lieu de jeter au 1er échec ; inserts idempotents (upsert offerId) → réessai sûr. Groupé avec A6/A7 dans `merchant.ts`.
...
- ~~**A6 [M×S] Pas de timeout fetch Google** + **A7 [M×S] `refreshGoogleToken` confond blip et révocation**~~ ✅ **FAIT (2026-07-05, run autonome)** — A6 : `AbortSignal.timeout` (30 s, borné au budget restant) sur tous les fetch Google (`googleMerchantFetch`/`refreshGoogleToken`/`exchangeGoogleCode`). A7 : `refreshGoogleToken` renvoie une union `{ok:true,tokens}|{ok:false,revoked,cause?}` — révocation EXPLICITE (400 `invalid_grant`) → « reconnexion requise » ; blip transitoire (réseau/5xx/config) → réessai auto, **jamais** « reconnexion requise » au marchand. `getGoogleAccessToken` = source unique du message + du signal Sentry (blip capturé avec cause, révocation silencieuse=statut DB). **Revue SF-hunter : 1 HIGH (retry deadline-unaware → débordement kill Vercel = troncature silencieuse) + 3 findings, TOUS corrigés** (deadline threadé via `{deadlineMs}` dans le cron, cause réseau préservée, Sentry dé-doublé, write token gardé). `tests/google-merchant-http.test.ts` (+25). 0 migration, réversible.
...
- **C3 [H×M] Double cascade externe/produit** (`lookupEan` 4 sources séq puis `collectAllEanSources` 4 sources //) → ÷2 appels. `enrich-product.ts:42,96`. **NB : P0-10 (correctude convergence) FAIT 2026-07-06 ; C3 = l'OPTIM COÛT restante** (unifier en UNE collecte multi-source qui applique l'enrichissement ET score — refonte du hot path, effort M).
