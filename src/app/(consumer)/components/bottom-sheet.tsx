...
export function BottomSheet({ merchants, isLoading, distLabel }: BottomSheetProps) {
    ...
    return (
        ...
        >
            <Drawer.Portal>
                <Drawer.Content
                    className="fixed bottom-0 left-0 right-0 z-40 flex max-h-[92dvh] flex-col rounded-t-3xl bg-[#2C1A0E] outline-none"
                ...
                >
                    ...
                    >
                        <div className="h-1 w-10 rounded-full bg-[#F5EDD8]/20" />
                    </div>

                    {/* Header */}
                    <div className="flex shrink-0 items-center justify-between px-5 pb-3">
                        <div>
                            <h2 className="text-sm font-bold text-[#F5EDD8]">
                                {isLoading
                                    ? "Recherche..."
                                    : `${merchants.length} boutique${merchants.length !== 1 ? "s" : ""}`}
                            </h2>
                            {!isLoading && merchants.length > 0 && distLabel && (
                                <p className="text-[11px] text-[#F5EDD8]/50">
                        ...
                        {snap === MIN_SNAP && merchants.length > 0 && (
                            <button
                                type="button"
                                onClick={() => setSnap("50%")}
                                className="text-xs font-semibold text-[#C17B2F]"
                            >
                                Voir tout
                            </button>
                        )}
...
function MerchantCard({ merchant }: { merchant: Merchant }) {
    ...
    return (
        ...
        >
            {/* Avatar */}
            <div className="flex size-13 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#2C1A0E] shadow-sm">
                {logo ? (
            ...
            <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 flex items-center gap-1 text-[11px] text-[#F5EDD8]/50">
                    ...
                    {minutes} min à pied
                    <span className="text-[#F5EDD8]/30">·</span>
