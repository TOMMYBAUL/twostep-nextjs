...
export default function ExplorePage() {
    ...
    return (
        <div className="relative h-full bg-[#e8eaed]">
            ...
            {viewMode === "liste" && (
                <div
                    className="absolute inset-0 z-20 overflow-y-auto bg-primary"
                    style={{ paddingTop: "calc(env(safe-area-inset-top) + 140px)" }}
                >
                    <div className="px-4 pb-24">
                        <p role="status" aria-live="polite" aria-atomic="true" className="mb-3 text-sm font-bold text-primary">
                            {isLoading
                                ? "Recherche..."
            ...
            >
                ...
                <div className="flex gap-2">
                    <div className={cx(
                        ...
                        searchFocused && "shadow-lg ring-2 ring-brand-secondary/20",
                    )}>
                        <SearchMd className="size-[18px] shrink-0 text-placeholder" aria-hidden="true" />
                        <input
                            type="text"
                            value={searchQuery}
                        ...
                        />
                        {searchQuery && (
                            <button type="button" onClick={() => setSearchQuery("")} className="text-placeholder hover:text-secondary">
                    ...
                    <button
                        ...
                        className={cx(
                            ...
                            radiusOpen ? "bg-brand-solid text-white" : "bg-primary text-primary",
                        )}
                    ...
                    <button
                        ...
                        className={cx(
                            ...
                            category ? "bg-brand-solid text-white" : "bg-primary text-primary",
                        )}
                        aria-label="Filtres"
                    >
                    ...
                    <div className="flex rounded-full bg-white p-1 shadow-md">
                        <button
                            ...
                            className={cx(
                                ...
                                viewMode === "liste" ? "bg-brand-solid text-white" : "text-primary",
                            )}
                        >
                            Liste
                        </button>
                        <button
                            ...
                            className={cx(
                                "rounded-full px-6 py-2 text-sm font-semibold transition duration-150",
                                viewMode === "carte" ? "bg-brand-solid text-white" : "text-primary",
                            )}
                        >
                            Carte
                        </button>
                    ...
                    <div role="listbox" aria-label="Suggestions" className="mt-2 overflow-hidden rounded-2xl bg-white shadow-xl">
                        {suggestions.map((s, i) => (
                            <button
                                ...
                                role="option"
                                aria-selected={false}
                                className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-sm text-primary transition duration-100 hover:bg-primary_hover"
                                onMouseDown={(e) => { e.preventDefault(); handleSearch(s.suggestion); }}
                            >
                                <span className="rounded-lg bg-secondary px-2 py-0.5 text-[10px] font-semibold text-tertiary">
                ...
                {radiusOpen && (
                    <div className="mt-2 overflow-hidden rounded-2xl bg-white p-4 shadow-xl ring-1 ring-black/5">
                        <p className="text-xs font-semibold text-primary">Rayon de recherche</p>
                        <div className="mt-3 flex items-center gap-3">
                            <input
                                ...
                                aria-label="Rayon de recherche"
                                className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-tertiary accent-brand-secondary [&::-webkit-slider-thumb]:size-5 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-brand-solid [&::-webkit-slider-thumb]:shadow-md"
                            />
                            <span className="w-12 shrink-0 text-right text-sm font-bold text-brand-secondary">{radius} km</span>
                        </div>
                        <div className="mt-1.5 flex justify-between text-[10px] text-placeholder">
                            <span>1 km</span>
                            <span>5 km</span>
                            <span>10 km</span>
                        </div>
                ...
                {filterOpen && (
                    <div className="ml-auto mt-2 max-h-[60vh] w-64 overflow-y-auto overscroll-contain rounded-2xl bg-white p-1.5 shadow-xl ring-1 ring-black/5">
                        ...
                        {/* Separator */}
                        <div className="mx-2 my-1.5 border-t border-tertiary" />

                        {/* Taille vêtements */}
                        {(availableSizes?.clothing?.length ?? 0) > 0 && (
                            <>
                                <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-placeholder">Taille</p>
                                <div className="flex flex-wrap gap-1 px-2 pb-2">
                                    {availableSizes!.clothing.map((s) => (
                                        <button key={s} type="button" onClick={() => setSizeFilterExplore(sizeFilterExplore === s ? null : s)}
                                            className={cx("rounded-md px-2 py-1 text-[11px] font-medium transition", sizeFilterExplore === s ? "bg-brand-solid text-white" : "bg-secondary text-secondary")}
                                        >{s}</button>
                                    ))}
                                </div>
                            </>
                        ...
                        {(availableSizes?.shoe?.length ?? 0) > 0 && (
                            <>
                                <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-placeholder">Pointure</p>
                                <div className="flex flex-wrap gap-1 px-2 pb-2">
                                    {availableSizes!.shoe.map((s) => (
                                        <button key={s} type="button" onClick={() => setShoeSizeFilterExplore(shoeSizeFilterExplore === s ? null : s)}
                                            className={cx("rounded-md px-2 py-1 text-[11px] font-medium transition", shoeSizeFilterExplore === s ? "bg-brand-solid text-white" : "bg-secondary text-secondary")}
                                        >{s}</button>
                                    ))}
                                </div>
                            </>
                        ...
                        {(availableSizes?.clothing?.length ?? 0) === 0 && (availableSizes?.shoe?.length ?? 0) === 0 && (
                            <p className="px-3 py-2 text-[11px] text-placeholder">Aucune taille renseignée pour le moment.</p>
                        )}
                    </div>
                )}
            </div>
                ...
                <div className="absolute right-3 z-10 flex flex-col gap-2" style={{ bottom: "calc(env(safe-area-inset-bottom) + 62px)" }}>
                    <button
                        ...
                        className={cx(
                            ...
                            is3D ? "bg-brand-solid text-white" : "bg-primary text-primary",
                        )}
                    ...
                    >
                        <NavigationPointer01 className="size-5 text-brand-secondary" aria-hidden="true" />
                    </button>
                </div>
            )}
                ...
                <div className="absolute left-4 right-4 z-30 mx-auto max-w-md animate-in slide-in-from-bottom-4 fade-in duration-200" style={{ bottom: "calc(env(safe-area-inset-bottom) + 62px)" }}>
                    <div className="overflow-hidden rounded-2xl bg-white shadow-xl">
                        <div className="flex items-center gap-3 p-4">
                            <div className="relative flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-secondary">
                                {selectedMerchant.merchant_photo || selectedMerchant.merchant_logo ? (
                                    <Image src={selectedMerchant.merchant_logo || selectedMerchant.merchant_photo || ""} alt="" fill sizes="56px" className="object-cover" />
                                ) : (
                            ...
                            <div className="min-w-0 flex-1">
                                <h3 className="truncate text-sm font-bold text-primary">{selectedMerchant.merchant_name}</h3>
                                <p className="mt-0.5 truncate text-xs text-tertiary">{selectedMerchant.merchant_address}</p>
                            ...
                            <Link
                                ...
                                className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-brand-solid text-white shadow-sm transition duration-150 active:scale-95"
                            >
...
function MerchantListCard({ merchant }: { merchant: NearbyMerchant }) {
    ...
    return (
        <div className="flex items-center gap-3 rounded-2xl bg-secondary p-3">
            ...
            >
                <div className="relative flex size-13 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary">
                    {logo ? (
                        <Image src={logo} alt="" fill sizes="52px" className="object-cover" />
                    ) : (
                        <span className="text-lg font-bold text-brand-secondary">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 truncate text-xs text-tertiary">{merchant.merchant_address}</p>
