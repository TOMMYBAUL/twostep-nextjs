...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    try {
        ...
        const [{ data: products }, { data: merchants }] = await Promise.all([
            supabase.from("products").select("slug, updated_at").eq("visible", true).eq("review_status", "validated").is("variant_of", null).order("updated_at", { ascending: false }).limit(1000),
