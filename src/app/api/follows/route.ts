...
export async function GET(request: NextRequest) {
    try {
        const limited = await rateLimit(request.headers.get("x-forwarded-for") ?? null, "follows", 30);
...
export async function POST(request: NextRequest) {
    try {
        const limited = await rateLimit(request.headers.get("x-forwarded-for") ?? null, "follows-post", 20);
        if (limited) return limited;
