...
2. Avance du travail RÉVERSIBLE sur le BACKLOG pré-autorisé (AUTONOMY.md §5) : Collecte ③ (stock) → ④ → ⑤ → Triage → Enrichissement → Stockage → Exploitation. Prends la PREMIÈRE sous-étape non terminée selon le worklog, fais-la, puis ENCHAÎNE la suivante (validation par lots, pas à chaque sous-étape). Commit après chaque petit pas. Cherche failles, angles morts, coûts cachés. IMPORTANT : si la sous-étape courante n'a plus que des restes BLOQUÉS (migration/design-gated, ou nécessitant un garde-fou dur), documente-les dans le worklog (commit) puis PASSE à la sous-étape suivante du backlog — ne t'arrête PAS pour si peu. Ne t'arrête QUE si TOUT le backlog est bloqué, ou si tu atteins un garde-fou dur, ou si un test casse.
