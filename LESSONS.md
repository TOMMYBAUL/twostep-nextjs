...
- ❌ **SELECT non borné → troncature SILENCIEUSE à 1000 lignes par PostgREST (défaut Supabase `max-rows`), SANS erreur.** Les 2 lectures de `ingestStockSnapshot` (index produits existants + lecture stock-en-cours de la réconciliation) lisaient `.select().eq()` sans `.range()` → sur un catalogue >1000 (Deerskin = milliers de SKU), les produits au-delà du 1000ᵉ étaient (a) absents de l'index → vus « nouveaux » → **DOUBLONS** (aucune contrainte UNIQUE sur `products(merchant_id, ean)`, seulement `slug`) ; (b) absents du set « en stock » → un vendu au-delà du 1000ᵉ **jamais remis à 0** = faux positif n°1. C'est la perte silencieuse la plus insidieuse : 0 erreur, invisible sur petits catalogues, mord à l'échelle pilote. Fix : helper `fetchAllRows(makeQuery, pageSize=1000)` (`src/lib/supabase/paginate.ts`) qui pagine via `.range()` jusqu'à une page < pageSize. **Règle : toute lecture `.select()` dont la liste peut dépasser ~1000 lignes DOIT paginer (`.range()` en boucle), symétrique au `chunk()` des écritures `.in()`.** Pagination = ordre DÉTERMINISTE obligatoire (`.order("id")`) — sinon, sous écriture concurrente (le call site `/api/catalog/import` n'a PAS de sync_lock, contrairement à `/api/ingest/stock`), une ligne peut tomber dans le trou entre 2 pages = doublon. **Fail-loud** : `data=null` sans erreur (anomalie SDK) → ERREUR synthétique (jamais index vide masqué → doublons, ni set partiel pris pour complet). Preuve : `tests/lib/supabase/paginate.test.ts` (loop/boundary/erreur-page-N/null-page) + `tests/ingest-snapshot-pagination.test.ts` (1500 produits, faux client qui plafonne à 1000 sans `.range` → 0 doublon + #1200 vendu au-delà de 1000 remis à 0). **✅ FAIT (2026-06-30) : les 4 lectures produits des SORTIES Google paginées** (Voie A cron `google-feed`, Voie B XML `feed/lfp/[merchantId]`, `google/feed-preview`, `google/inventory` `pushInventoryToGoogle`) — même trou, `fetchAllRows(() => ….order("id",{ascending:true}))`. Sur >1000 chaque sortie publiait un feed PARTIEL silencieux (inventory : un vendu au-delà du 1000ᵉ resté « in stock » sur Google = faux positif n°1). Contrat `{data,error}` préservé → gardes callers inchangées ; inventory garde `.in("id",productIds)` (push ciblé) dans la factory. Preuve `tests/google-feed-output-pagination.test.ts` (catalogue 1500 → `.range()` 2 pages sur les 4 + compteurs aval 1500, non vacant) + 2 faux clients existants mis à jour (`.order`/`.range`). 959→963. Revue SF-hunter **SOUND, 0 finding**. **Reste SCALE : mémoire `lfp-xml` (XML 50k en RAM), timeouts crons/routes Vercel.** (SCALE/VOLUME, paginate.ts, 2026-06-30)
- ❌ **Une SORTIE volumineuse matérialisée en RAM = risque mémoire à l'échelle ; la passer en STREAMING change le contrat fail-loud.** Le feed XML Voie B (`feed/lfp/[merchantId]`) faisait `fetchAllRows` (tout le tableau) puis `buildLfpXml` `.map().join("")` (toute la chaîne) → 50k items = 3 copies résidentes. Rendu streaming via `streamRows` (async generator lazy, `src/lib/supabase/paginate.ts`) + `ReadableStream` (head→items page par page→tail ; `buildLfpXml` scindé en `lfpXmlHead`/`lfpXmlItem`/`lfpXmlTail`, source unique du format → byte-identique sur feed non vide). **Piège north-star** : une fois le 1er octet émis, le statut HTTP 200 est figé → une erreur de lecture sur une page ULTÉRIEURE ne peut plus devenir un 500 ; elle DOIT `controller.error()` (avorter le transfert → Google re-crawle) et JAMAIS `controller.close()` sur un corps tronqué (= 200 « complet » silencieusement amputé = perte n°1). **Règles** : (1) **peek de la 1re page AVANT de construire la Response** → erreur initiale = vrai 500 propre ; (2) `streamRows` est **fail-loud par THROW** (≠ `fetchAllRows` qui rend `{data,error}` : un consommateur déjà en train d'émettre doit avorter, pas masquer) ; (3) l'abort en `try { captureError } finally { controller.error }` — si le report d'erreur lève, l'abort doit quand même s'exécuter (sinon flux pendant). Preuve sans yeux : driver le VRAI GET avec un faux client qui ERRORE sur la page N → `res.text()` doit REJETER (corps avorté), jamais résoudre un XML tronqué. `tests/feed-lfp-stream.test.ts` + `streamRows` dans `paginate.test.ts`. Revue SF-hunter SOUND. (SCALE/VOLUME, feed-lfp streaming, 2026-06-30)
- ❌ **Un cron qui boucle N appels réseau SÉQUENTIELS sans `maxDuration` ni budget temps = troncature SILENCIEUSE à l'échelle.** `cron/google-feed` poussait tous les produits de tous les marchands en une invocation (1 `googleMerchantFetch` par produit) → sur un catalogue pilote (milliers de SKU) le kill Vercel coupe la fonction en plein vol : produits restants OMIS **+ l'écriture de `last_feed_status` (fin de boucle) JAMAIS atteinte** → le marchand garde le « success » du run précédent = feed partiel sans aucun signal. Fix : `export const maxDuration = 300` + budget auto-imposé `TIME_BUDGET_MS` SOUS maxDuration via helper PUR `processWithinTimeBudget(items, action, {now, deadlineMs})` (horloge injectée → testable sans réseau) qui s'arrête PROPREMENT et renvoie `{succeeded, attempted, interrupted}` → l'appelant écrit « partial » honnête (jamais faux « success ») + Sentry sur toute interruption. **Règles** : (1) tout cron qui itère un volume non borné DOIT poser `maxDuration` + un budget temps EN-DEÇÀ (marge pour finaliser/écrire le statut) ; (2) une INTERRUPTION par budget doit produire un statut DB honnête + un signal Sentry — sinon c'est la perte n°1 ; (3) le signal d'escalade doit tirer même sur le pilote MONO-marchand (`merchantsAttempted===length` ≠ « rien sauté » → garder la condition sur `budgetExhausted`, pas sur `attempted<length`) ; (4) bonus revue : les `.update` de statut doivent destructurer `error` (un write avalé ré-introduit le faux « success » par le chemin écriture). Preuve sans horloge réelle : helper avec `now()` injecté + route drivée avec `Date.now` spié qui avance à chaque push. `src/lib/google/feed-push.ts`, `tests/google-feed-time-budget.test.ts`. Revue SF-hunter 3 findings (2 corrigés, 1 skip scope). (SCALE/VOLUME, google-feed timeout, 2026-07-01)
- ⚠️ **STREAMER une lecture pour borner la mémoire borne la MÉMOIRE, PAS le TEMPS — et sous un push LENT, ça
  AMPLIFIE la dérive de pagination OFFSET.** Passé le cron `google-feed` (Voie A) de `fetchAllRows` (tout le
  catalogue en RAM pendant ~270 s) à `streamRows` (1 page à la fois) via `processStreamWithinTimeBudget` (le
  `processWithinTimeBudget` array délègue via `singlePage` → 1 seule sémantique budget). **La note de backlog
  « streamer pour finir un catalogue > budget en UN run » était FAUSSE** : le budget est borné par N appels
  réseau séquentiels à Google, pas par la lecture DB → un catalogue trop gros reste "partial"+tail au run
  suivant, streaming ou pas. Vrai gain = mémoire bornée + pages non poussables (interruption) non lues. **PIÈGE
  révélé par SF-hunter** : `streamRows` pagine par OFFSET (`.range()`) ; avec un push LENT la lecture s'étale sur
  ~270 s → écriture concurrente sur une page déjà lue = produit sauté en silence. **MAIS transient** : le cron
  re-pousse le catalogue COMPLET idempotent chaque run → réapparaît au suivant (≠ ingest-snapshot où skip→doublon
  PERMANENT). **Règles** : (1) ne pas streamer « pour la mémoire » sans mesurer que la mémoire est le VRAI
  bottleneck (Voie A = objets produits ~5 MB à l'échelle pilote, pas une chaîne XML 50k comme Voie B) ; (2) une
  lecture paginée OFFSET interleavée avec un traitement LENT doit passer en KEYSET (`WHERE id > dernier ORDER BY
  id`, drift-immune) ; (3) `products_pushed` du chemin catch doit refléter les push RÉELS de CE run (compteur
  incrémenté dans l'action), pas le stale du run passé ; (4) une garde de deadline par-ITEM ne peut PAS devenir
...
- ❌ **Une boucle d'écriture PAR PRODUIT (N aller-retours réseau sériés) = troncature silencieuse à l'échelle.**
  ...
  → produits restants OMIS + réconciliation/enfilage (fin de fonction) jamais atteints = perte n°1. Fix : DEUX PHASES — boucle
  de PLAN (pure pour les créations) → FLUSH par LOTS de 500 (`chunk()`), ordre products→stock→feed→enrich (FK). **Pièges du
  batching à ne pas rater** : (1) dédup INTRA-push — un groupe qui matche un produit créé plus tôt dans le même push doit
  fusionner dans la ligne d'insert PENDING (`insertRowById`), pas un UPDATE DB sur une ligne pas encore écrite (no-op silencieux) ;
  (2) stock DÉDUP par product_id (Map, dernier gagne) sinon Postgres « ON CONFLICT ne peut affecter 2× la même ligne » sur un
  create+update du même produit dans un lot ; (3) repli ISOLANT mono-ligne sur un lot d'insert en échec (INSERT transactionnel →
  1 slug FR collision « café »/« cafe » ne doit pas tuer 499 saines) ; (4) `stock_replaced` = lignes RÉELLEMENT écrites, produit
...
- ❌ **`captureError(objet Supabase/PostgREST)` → « [object Object] » en Sentry** : un PostgrestError n'est pas `instanceof
  ...
  `extra`). `tests/lib/error.test.ts`. (revue SF-hunter maillon SCALE, 2026-07-01)
- ❌ **Un échec de write NON-critique qui `continue` peut faire zéroïser le produit par la réconciliation** : dans
  `ingestStockSnapshot`, l'échec de la MAJ métadonnée (prix/tailles) d'un produit pré-existant faisait `continue` → produit hors
