-- Fix: migration 035 accidentally created a second overload of get_merchants_nearby
-- with DOUBLE PRECISION radius_km instead of INT, breaking the API.
-- This drops the broken overload and recreates the correct function with LOWER() fix.

-- Drop the broken DOUBLE PRECISION overload created by 035
DROP FUNCTION IF EXISTS get_merchants_nearby(DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, INTEGER, TEXT);

-- Recreate the correct function (from 034) with LOWER() category comparison
