-- Fix: size filter must check quantity > 0 in available_sizes JSONB
-- Before: @> '[{"size":"45"}]' matched even when quantity = 0
-- After: EXISTS checks both size match AND quantity > 0
